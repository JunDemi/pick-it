...
function EditGameInfo(prop: {
  ...
  //props값을 상태에 할당
  useEffect(() => {
    setTitle(prop.gameData.gameInfo.worldcupTitle);
    setDescription(prop.gameData.gameInfo.worldcupDescription);
    setCategoryArray(prop.gameData.gameInfo.category);
  }, []);
    ...
    <>
      ...
      <AnimatePresence>
        {thumbnailPopup && (
          <motion.div
            className={
              (prop.gameData.gameInfo.worldcupImages.length > 8 &&
                "scroll-hide") + " edit-thumbnail-popup"
            }
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <div className="thumbnail-container">
              ...
              <div className="img-wrapper">
                {prop.gameData.gameInfo.worldcupImages.map(
